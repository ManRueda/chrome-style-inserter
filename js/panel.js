...
function StyleInserter(editor) {
  ...
  this.showError = function(err) {
    this.editor.querySelector('.error').textContent = err;
  ...
  this.compile = function(cb) {
    ...
    that.clearError();
    if (that.type === 'less') {
      less.render(that.preCompiled, {}).then(function(out) {
        that.css = out.css;
        cb(that.css);
      }, function(err) {
        if (err) {
          that.showError(err.type + ': ' +err.message);
        }
      });
    } else if (that.type === 'sass'){
      sass.compile(that.preCompiled, function(result) {
        if (result.status === 0){
          that.css = result.text;
          cb(that.css);
        }else{
          that.showError(result.formatted);
        }
      });
    }else{
      cb(that.css);
  ...
  this.getInsertScript = function() {
    var template = (function() {
      var styles = document.querySelectorAll('head style[data-styleinserter="true"][data-id="##ID##"]');
      ...
      tag.dataset.styleinserter = true;
      tag.dataset.id = '##ID##';
  ...
  that.editor.querySelector('.code').addEventListener('keyup', function(argument) {
    ...
    that.compile(function(css) {
      ...
      if (that.autoInsert) {
        that.insertStyle(function() {}, false);
      }
    });
  });
}
